...
include("./connect.php");
$query = "SELECT `Item_id`, `brand_name`, `Item_name`, `item_img`, `item_desc`, `Item_price`, `type`, `date`, `s_id` FROM `items` WHERE `type` = 'stationary'";
    ...
    if ($run == true) {
        while ($data = mysqli_fetch_assoc($run)) {
            ...
            <div style=" margin-top:2%; border:1px solid black;box-shadow:4px 4px 4px grey;margin-right:4%; display:grid; grid-template-columns: 30% 70%; ">
                <form action="staionary_sql.php" method="post">
                    <div style="padding:5%">
...
<head>
    ...
    <title>webcart| Stationary Page</title>
